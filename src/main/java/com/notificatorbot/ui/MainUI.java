package com.notificatorbot.ui;

import java.time.format.DateTimeFormatter;
import java.util.List;

import com.notificatorbot.bot.NotificatorBot;
...
import com.notificatorbot.ui.components.AlertEvent;
import com.notificatorbot.ui.components.UncleMessage;
import com.notificatorbot.ui.components.UncleTab;
import com.notificatorbot.utils.BotUtils;
...
public class MainUI extends VerticalLayout {
    ...
    private void sendMessage_handler(ClickEvent<Button> event) {
        if (dgdUserAccount.getSelectedItems() != null && !dgdUserAccount.getSelectedItems().isEmpty()) {
            ...
            message.addAcceptListener((ComponentEventListener<AlertEvent>) event1 -> {
                Long[] chatIds = dgdUserAccount.getSelectedItems().stream().filter(user ->
                user.getActive()==true).map(p -> p.getChatId()).toArray(Long[]::new);

                BotUtils.sendMessage(notificatorBot, event1.getMessage(), chatIds);
            });
